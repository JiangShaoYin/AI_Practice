...
def test():
    with tf.Graph().as_default() as g:                                      #复现之前定义的计算图，并执行以下操作
        ...
        ema = tf.train.ExponentialMovingAverage(cifar10_backward.MOVING_AVERAGE_DECAY)# 实现滑动平均模型，参数MOVING_AVERAGE_DECAY用于控制模型更新的速度，训练过程中会对每一个变量维护一个影子变量
        ...
        img_batch,label_batch = cifar10_generateds.get_tfrecord(TEST_NUM, isTrain=False)  #2 一次批获取 TEST_NUM 张图片和标签
